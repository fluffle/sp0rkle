...
func (data Data) String() string {
	...
	fmt.Fprintf(b, "%s flight %s/%s", data.Airline.Name, data.Flight.IATA, data.Flight.ICAO)
	// from George Bush Intercontinental (IAH) gate E9
	fmt.Fprintf(b, " from %s (%s)", data.Departure.Airport, data.Departure.Code)
	if data.Departure.Terminal != "" && data.Departure.Gate != "" {
		fmt.Fprintf(b, " gate %s%s", data.Departure.Terminal, data.Departure.Gate)
	}
	...
	if data.Arrival.Terminal != "" && data.Arrival.Gate != "" {
		fmt.Fprintf(b, " gate %s%s", data.Arrival.Terminal, data.Arrival.Gate)
...
func (data *Data) liveStatus() string {
	...
	if data.Live.Ground {
		return " is waiting to take off."
	}
	return fmt.Sprintf(" is flying at %.0f km/h, altitude %.2f km.",
		data.Live.Speed, data.Live.Altitude / 1000)
}
